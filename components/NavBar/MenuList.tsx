...
export function MenuList({ data, label }: MenuListProps) {
  const [isOpen, setIsOpen] = React.useState(false); // State to control menu open/close

  const handleMouseEnter = () => setIsOpen(true);  // Open the menu on hover
  const handleMouseLeave = () => setIsOpen(true); // Close the menu when not hovering
  ...
  return (
    ...
    >
      ...
      {/* Conditionally render the content based on isOpen */}
      {isOpen && (
        <NavigationMenuContent >
